...
{ 
  ...
  while (1)
  {
    ...
    if(convert){
     To_Fahrenheit(Decimal, Temperature_H);
    }

    //display FFFF at standby mode
    if(stand_by){
      Send7SEG(4,0x71);
      Send7SEG(3,0x71);
      Send7SEG(2,0x71);
    ...
    //set flag based on the control signal
    if (control == 'f'){
      convert = true;
    }
    if (control == 'c'){
...
/***************************************************************************
 Function Name: To_Fahrenheit

 Purpose: 
   Convert temperature from Celsius to Fahrenheit
****************************************************************************/
void To_Fahrenheit(int& Decimal, byte& High)
{
...
void UpdateRGB (byte Temperature_H)
{
  ...
  if (change_light) {        
      digitalWrite(BLUE, HIGH); /* change light to blue */
      change_light = false;    
   }
  else{
